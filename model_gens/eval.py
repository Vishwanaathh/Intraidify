...
LLM PROVIDER: uses Gemini 2.5 Flash-Lite. gemini-2.0-flash was retired
June 1, 2026 (this is why it was returning 429/quota-exhausted errors with
limit: 0 - it's not a quota issue, the model itself is gone). 2.5 Flash-Lite
is Google's official migration target: same free pricing, and currently
Gemini's most generous free-tier model (15 RPM, 1,000 requests/day) -
comfortably covers the full ~398-day test set in one run. No new setup
needed beyond your existing GEMINI_API_KEY.
...
This script now also computes and reports "cascade_top10": for each day,
...
Note: this field only populates for days evaluated AFTER this update -
older checkpoint entries won't have it and are excluded from this specific
aggregate (but still count toward the secondary full-list metrics).

QUOTA NOTE: MAX_LLM_CALLS_PER_RUN below controls how many new LLM calls
happen per run, then the script stops and saves a checkpoint. With 2.5
Flash-Lite's 1,000/day quota, you can set this to 400+ and cover the
entire ~398-day test set in a single run.
"""
...
with step(f"run pipeline ablation, up to {MAX_LLM_CALLS_PER_RUN} new LLM calls this run"):
    for day_i, row in pipeline_test_df.iterrows():
        ...
        if day_key in completed_days:
            continue  # already done in a previous run - skip, no API call spent
...
with step("aggregate pipeline ablation results (cumulative, all completed days)"):
    def agg(key):
        rhos = [d[key]["spearman"] for d in completed_days.values() if not pd.isna(d[key]["spearman"])]
        ndcgs = [d[key]["ndcg_10"] for d in completed_days.values()]
...
with step("paired significance tests (Wilcoxon signed-rank, same days across configs)"):
    def paired_test(key_a, key_b, metric):
        vals_a = [d[key_a][metric] for d in completed_days.values()]
        vals_b = [d[key_b][metric] for d in completed_days.values()]
    ...
    pipeline_metrics["known_limitations"] = [
        ...
        "placeholder), which softmax treats as a real competing score.",
    ]
